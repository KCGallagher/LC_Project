...
nem_order_param_values = np.zeros(len(time_range))
smec_order_param_values = np.full(len(time_range), np.nan)
...
for i, time in enumerate(time_range):  # interate over dump files
    ...
    nem_order_param_values[i] = nematic_order_param(
        rod_positions
    )  # evaluate order param at time t
    smec_order_param_values[i] = smectic_order_param(
        rod_positions[:, 1]
    )  # use CoM to find smectic order param
...
ax1.set_ylabel(r"Order Parameter ($S$)")
ax1.plot(
    time_range,
    uniform_filter1d(nem_order_param_values, size=int(10)),
    label=r"Nematic Order ($S_{n}$)",
    color="tab:blue",
)
ax1.plot(
    time_range,
    uniform_filter1d(smec_order_param_values, size=int(10)),
    label=r"Smectic Order ($S_{s}$)",
    color="tab:green",
)
...
color = "tab:blue"
ax2.set_ylabel("Volume Fraction")  # we already handled the x-label with ax1
ax2.plot(
    time_range,
    vol_frac(volume_values),
    linestyle="--",
    label=r"Volume Fraction ($\phi$)",
    color="k",  # black
)
...
plt.plot(vol_frac(volume_values), smec_order_param_values, "gx", label="Smectic Order")
plt.ylabel("Order Parameter")
plt.xlabel("Volume Fraction")
plt.savefig("comb_order_vs_volfrac.png")
plt.show()
