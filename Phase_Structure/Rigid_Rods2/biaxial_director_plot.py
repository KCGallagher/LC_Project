...
def order_param(data):
    ...
    number_of_nans = np.count_nonzero(np.isnan(M_matrix))
    if number_of_nans > 0:
        print("Warning: " + str(number_of_nans) + " NaN values removed")
    M_eigen = np.linalg.eigvals(np.nan_to_num(M_matrix))  # replaces nan with zero
...
for i, time in enumerate(time_range):  # interate over dump files
    ...
    """Indices are Molecule Number/ First (0), Middle (1), or Last (2) atom,/ Positional coord index"""
