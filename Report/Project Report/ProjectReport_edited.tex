...
where $P_{2}$ denotes the second Legendre polynomial \cite{DeGennes1993}. While $\langle \cos^{2}(\theta) \rangle$ would function alone as the order parameter, this confines $S_{n}$ to values between unity for a perfectly aligned system, and zero for a completely random system. Further information on the motivation behind this choice, and how it is implemented in our simulation, is given in Appendix \ref{sec:NematicOrderAppendix}.
