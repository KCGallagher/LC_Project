...
%DIF 6a6-10
 %DIF > 
\PassOptionsToPackage{dvipsnames,svgnames}{xcolor} %DIF > 
\usepackage[dvipsnames]{xcolor} %DIF > 
\definecolor{forestgreen}{RGB}{34,139,34} %DIF > 
 %DIF > 
%DIF -------
\input{structure.tex} % Specifies the document structure and loads requires packages
%DIF 7c12
%DIF < \graphicspath{{"/Users/kit gallagher/Documents/Research Review/Report/Figures"}}
%DIF -------
\graphicspath{{"/Users/kit gallagher/Documents/Reseaerch Review/Report/Figures"}} %DIF > 
%DIF -------
...
Since Kirkwood's prediction of an \DIFdelbegin \DIFdel{entropy-driven }\DIFdelend \DIFaddbegin \DIFadd{entropically driven }\DIFaddend ordering transition in the freezing of hard spheres \cite{Kirkwood1954}, \DIFdelbegin \DIFdel{entropy-driven }\DIFdelend \DIFaddbegin \DIFadd{entropically driven }\DIFaddend phase transitions have been the subject of much attention \cite{Kerr1993,Frenkel1999}. Their relevance to colloidal systems is often underestimated, but gives rise to a range of diverse phase behaviour \cite{Adams1998, Anderson2002, Forsyth1978}. In particular, there is a wealth of (primarily experimental) studies of `bent-core' liquid crystals \DIFdelbegin \DIFdel{-- }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend rigid rods with a central bend \cite{Takezoe2006, Etxebarria2008, Yang2018}.

These molecules have primarily been used for their polarity however; here we consider a bent-core system with no polarity, \DIFdelbegin \DIFdel{which displays true entropy-driven }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{which displays true} entropically driven }\DIFaddend phase transitions. We conduct molecular dynamics simulations on a course-grained model to study such transitions in a system of ‘nunchucks’, molecules consisting of two rigid rods of double stranded (ds-) DNA, connected via a flexible linker of single stranded (ss-) DNA. This linker allows the configuration to vary from fully stretched to folded. 

DNA is chosen due to \DIFdelbegin \DIFdel{availability through polymerase chain reactions, its exact design in terms of sequence and thus its }\DIFdelend \DIFaddbegin \DIFadd{its \textcolor{forestgreen}{availablity through PCR, its exact design in terms of sequence and thus its} }\DIFaddend reproducibly on the nanoscale. The techniques in phase simulation developed here are relevant to more complex DNA structures, such as in the development of liquid crystal behaviour in DNA origami \cite{Cha2015,Wang2018}, or in \DIFdelbegin \DIFdel{entropy-driven }\DIFdelend \DIFaddbegin \DIFadd{entropy driven }\DIFaddend nano-structure self-assembly \cite{Barry2010, Lin2000}.
...
%DIF < \footnotemark \ %removed from after molecules
...
Liquid crystals can also be categorised by the driving force behind their transition. \DIFdelbegin \DIFdel{Samples only containing one type of smaller molecules undergo thermally-driven phase transitions, and are called thermotropic liquid crystals. By contrast, larger molecules or colloids dispersed in a continuous solvents are classed as lyotropic; their phase transitions are concentration driven \mbox{%DIFAUXCMD
\cite{DeGennes1993}}\hspace{0pt}%DIFAUXCMD
. Here I focus lyotropic, entropy-driven phase transitions, where the mesogens (here DNA-nanocolloids) interact purely repulsively. Our studies represent a benchmark for designer-made, DNA-based systems.
}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{Samples containing only one type of smaller molecules undergo thermally driven phase transitions, which are called thermotropic liquid crystals. By contrast, larger molecules or colloids dispersed in a continuous solvent are called lyotropic; their phase transitions are concentration driven \cite{DeGennes1993}. Here I focus lyotropic phase transitions that are entropically driven while the mesogens (here DNA-nanocolloids) interact purely repulsively. Our studies represent a benchmark for designer-made, DNA\textendash-based systems.}
}\DIFaddend 

\DIFdelbegin \DIFdel{Entropy-driven phase transitions }\DIFdelend \DIFaddbegin \DIFadd{Entropically driven \textcolor{forestgreen}{phase-transitions} }\DIFaddend are often seen as counter-intuitive, as such transitions rely on an increase in microscopic disorder from an increase in visible order (an example is given in Section \ref{sec:OnsagerTheory}). Once considered an abnormality, Frenkel famously noted that ``\textit{...such phase transformations may not be interesting exceptions, but the rule!}'' \cite{Frenkel1999}.

\subsection{Onsager Theory} \label{sec:OnsagerTheory}

Onsager predicted a simple model of lyotropic, entropy-driven phase transitions \cite{Onsager1949}. Rod-like particles are modelled as thin spherocylinders with length $L$ and \DIFdelbegin \DIFdel{diameter }\DIFdelend \DIFaddbegin \DIFadd{diamater }\DIFaddend $D$, and an excluded volume preventing overlap. The interaction potentials between \DIFdelbegin \DIFdel{the particles }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{the particles} }\DIFaddend are neglected, and only the configurational entropy of the system considered. Qualitatively, the confinement of rods \DIFdelbegin \DIFdel{parallel to a director $\Vec{n}$}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{parallel to a director $\Vec{n}$}}\DIFaddend , such as in the nematic phase, leads to a decrease in \DIFdelbegin \DIFdel{rotational }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{rotational} }\DIFaddend entropy, but an increase in \DIFdelbegin \DIFdel{translational entropy allowing rods to move freely parallel to each other}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{transltional} entropy \textcolor{forestgreen}{allowing rods to move parallel to each other}}\DIFaddend . At sufficiently high concentrations, the \DIFdelbegin \DIFdel{translational }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{rotational} }\DIFaddend entropy will dominate, and the system \DIFdelbegin \DIFdel{undergoes }\DIFdelend \DIFaddbegin \DIFadd{undergo }\DIFaddend a phase transition from isotropic to nematic. 

The critical volume fraction at transition is approximately $4r/L$, where $r$ is the radius of the rigid rod, and $L$ its length (see Appendix \ref{sec:OnsagerAppendix}). This means the isotropic--nematic phase transition occurs at lower concentrations for mesogens with greater aspect ratio, and requires a minimum aspect ratio of $L/r = 4$ to occur. It is worth noting that this result is only valid in the limit of thin spherocylinders, as Onsager's derivation neglects Virial coeffecients $B_{n}$ beyond second order in the expansion of the free energy in powers of density \cite{Frenkel1987}, \DIFdelbegin \DIFdel{as discussed in Appendix \ref{sec:OnsagerAppendix}}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{as discussed in Appendix \ref{sec:OnsagerAppendix}}}\DIFaddend . Onsager's theory has since been tested, with both computational \cite{Frenkel1987b, Frenkel1988b} and experimental \cite{Kubo1979,Oldenbourg1988,Fraden1993} results substantiating its validity.
...
The degree of order in a liquid crystal phase is characterised by an order parameter, chosen such that it \DIFdelbegin \DIFdel{is unity in the fully ordered phase and zero in the isotropic phase. An example is the magnetisation $\textbf{M}$ }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{is $S=1$ in the fully ordered and zero the isotropic phase}. \textcolor{forestgreen}{An example is the magnetisation $\textbf{M}$} }\DIFaddend of a ferromagnet; when raised above a critical temperature, the magnetisation vanishes as the ferromagnet undergoes a phase transition. While the choice of order parameter for the nematic phase transition is less intuitive than this, it relies on the formation of genuine long-range orientational order. We may therefore define the angle ($\theta$) between each molecule's axis and the system \DIFdelbegin \DIFdel{director, such that the nematic order parameter $S_{n}$ can be written as:
}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{director, such that the order parameter S can be written as:}
}\DIFaddend 

\begin{equation}
S_{n} = \left\langle P_{2}(\cos(\theta)) \right\rangle = \left\langle \frac{3}{2}\cos^{2}(\theta) - \frac{1}{2} \right\rangle \DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}

where $P_{2}$ denotes the second Legendre polynomial \cite{DeGennes1993}. While $\langle \cos^{2}(\theta) \rangle$ would function alone as the order parameter, \DIFdelbegin \DIFdel{this confines $S_{n}$ to values between unity for a perfectly aligned system, and zero for a completely random system. Further information on the motivation behind this choice, and how it is implemented in our simulation, is given in Appendix \ref{sec:NematicOrderAppendix}}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{this confines $S_n$ to values between unity for a perfectly aligned system, and zero for a completely random system. Further information is given in} Appendix \ref{sec:OrderParamTheory}}\DIFaddend .
...
All simulations in this report were conducted \DIFaddbegin \DIFadd{\textcolor{forestgreen}{for} }\DIFaddend a system of 1000 particles, with a time step of $0.005\tau$, (where $\tau$ is the characteristic time), unless otherwise stated. The system was initially configured in a dilute, isotropic state; a non-trivial process for large numbers of mesogens as molecules must be placed randomly without overlap, to prevent any initial order affecting the \DIFdelbegin \DIFdel{subsequent }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{subsequent} }\DIFaddend formation of ordered phases. \DIFdelbegin \DIFdel{When studying high volume fractions, simulations were }\DIFdelend \DIFaddbegin \DIFadd{I \textcolor{forestgreen}{thank} Iria Pantazi for writing a python script to automate this process for dilute rigid rod systems, and a generalised version of this is available in the supplementary material. \textcolor{forestgreen}{When studying high volume fractions}, simulations were also }\DIFaddend initiated from a perfectly ordered square crystalline phase, with all molecules aligned along a common axis. The choice of this axis is arbitrary, as the system is invariant under global rotation \cite{Nos1983}, but is taken to be directed along the \DIFdelbegin \DIFdel{$y$}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{$y$}}\DIFaddend -axis for clarity. Care was taken to ensure molecules did not overlap, and \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{that} }\DIFaddend the system was stable in this ordered phase.
%MAYBE INCLUDE CODE IN SUPP MATERIAL?? THINK ABOUT THIS!

All simulations are conducted within an oblong box defined by the Cartesian axes, with periodic boundary conditions \cite{Frenkel2002}. The aspect ratio of this box may be varied, to support phase formation in anisotropic systems, as discussed in Section \DIFdelbegin \DIFdel{\ref{sec:PairWise_Application}}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{X}}\DIFaddend . An isenthalpic ensemble was used (where pressure is fixed) to vary the size of the simulation region (either contracting or expanding), allowing sampling of different volume fractions from the same initial configuration. The microcanonical \DIFdelbegin \DIFdel{$(N,V,E)$ ensemble, with conserved volume and energy, }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{$(N,V,E)$ ensemble, with conserved volume and energy}, }\DIFaddend was then used to allow the system to reach thermodynamic equilibrium. Time integration was evaluated using the Nose-Hoover thermostat \cite{Nos1984, Hoover1985} natively implemented in LAMMPS \cite{Shinoda2004}, with a damping time of $\tau$.
...
%DIF > can cite frenkel on entropically drive transitions if req
...
The volume fraction and nematic order parameter were computed for comparison with Onsager's theorem, as detailed in Section \ref{sec:OnsagerTheory}. Calculation of the order parameter is complicated by the absence of an imposed director (\DIFdelbegin \DIFdel{i.e. }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{i.e.} }\DIFaddend if no electric field is applied), and we use the approach taken by Eppenga and Frenkel \cite{Eppenga1984} which is reproduced in Appendix \ref{sec:OrderParamCalc}. %or dussi 2018 or many others

Further analysis included the calculation of the smectic order parameter, and pair-wise orientational correlation coefficient, detailed in Section \ref{sec:PairWise_Application}. All scripts for data extraction and analysis were written by the author, \DIFdelbegin \DIFdel{and is attached as the supplementary material}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{\textit{and is attached as supplementary material}}}\DIFaddend .
...
\DIFdelbegin \DIFdel{First we studied a system of rigid rods to verify that the analysis methods we employed were able to reproduce Onsager's theory.
We focused specifically on the isotropic-to-nematic phase transition, as it is well studied, and verified computationally through both Monte Carlo (MC) \mbox{%DIFAUXCMD
\cite{Frenkel1984, Lee1987} }\hspace{0pt}%DIFAUXCMD
and MD simulations \mbox{%DIFAUXCMD
\cite{Allen1987, Camp1996}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{First we studied a system of rigid rods to verify that the analysis methods we employed was able to reproduce Onsager's theory. We focused specifically on the isotropic-to-nematic phase transition, as it is well studied, and verified computationally through both Monte Carlo (MC) \cite{Frenkel1984, Lee1987} and MD simulations \cite{Allen1987, Camp1996}}.
}\DIFaddend %not experimentally; long-range maier-snape is better here https://pubs.acs.org/doi/pdf/10.1021/j100303a008

\subsection{\DIFdelbegin \DIFdel{Shrinking the Simulation Box}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{Schrinking the Simulation Box}}\DIFaddend }

For rigid rods with an aspect ratio $L/D = 10$, \DIFdelbegin \DIFdel{Onsager's theory predicts a lyotropic phase transition at a critical volume fraction of $\phi  = 0.4$. Considering a system of }%DIFDELCMD < \num{1000} %%%
\DIFdel{rigid rods, formed of }%DIFDELCMD < \num{10} %%%
\DIFdel{sequentially connected `balls' (force-centres), we applied alternating stages of contraction (increasing the volume fraction) and equilibration steps (keeping the volume constant). }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{Onsager's theory predicts a lyotropic phase transition at a critical volume fraction of $\phi  = 0.4$. Considering a system of \num{1000} rigid rods, formed of \num{10} sequentially connected `balls' (force-centres), we applied alternating stages of contraction (increasing the volume fraction) and equilibration steps (keeping the volume constant)}. }\DIFaddend Each contraction stage consists of between \num{1e4} and \num{5e4} steps (chosen to identify the critical volume fraction with maximal resolution, while verifying the order parameter is approximately constant outside of this), while the equilibration stage runs for \num{2e6} steps. In this way we are able to confine the possible critical volume fraction to the range $0.39 < \phi< 0.44$, as observed in Figure \ref{fig:rr_nemorderparam}, in good agreement with Onsager's prediction.
...
The isotropic phase formation was observed in the region $0.38 < \phi< 0.41$, in good agreement with the results of Section \ref{sec:RigidRodSims}, \DIFdelbegin \DIFdel{and }\DIFdelend confirming this is indeed an equilibrium phase transition. \DIFdelbegin %DIFDELCMD < 
...
\DIFdelbegin \DIFdel{Therefore, I considered systems where the nunchuck opening angle is }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{Therefore, we considered systems for which the nunchuck opening angle was} }\DIFaddend confined to a particular value. \DIFdelbegin \DIFdel{I suggest that, in comparison to the fixed rigidity case, quasi-nematic structures may be observed on shorter length scales, particularly when this opening angle is confined below the mean angle (see Figure \ref{fig:nun_fr_angledist})}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{We suggest that similar to the rigid-rods, quasi-nematic structures may be observed on shorter length scales, particularly, when this opening angle is confined below the mean angle (see Figure \ref{fig:nun_fr_angledist})}}\DIFaddend .
We tested phase formation over a wide range of rigid rod lengths and opening angle values; broadly we found that larger opening angles favoured nematic-like phase formation\DIFdelbegin \DIFdel{(with a maximal order parameter of $S_{n} = 0.62$), }\DIFdelend \DIFaddbegin \DIFadd{, \textcolor{forestgreen}{for which} our analysis methods were best suited to identify, }\DIFaddend but smaller angles did not form obvious alternate ordered phases. Two such quasi-nematic phases observed are depicted in Figure \ref{fig:nun_fa_views}.
...
To determine order over longer length scales, without increasing the volume of the system (due to the limited computational resources), we instead vary the aspect ratio of the system. By considering an elongated oblong simulation region, such as in Figure \ref{fig:nun_fa_twist}, we are able to sample correlations over longer distances (along the long axis of the box) for systems of the same particle number. Using this method, \DIFdelbegin \DIFdel{the curves in Figure \ref{fig:num_fa_correlation_mol} confirm }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{the curves in Figure \ref{fig:num_fa_correlation_mol} confirm} }\DIFaddend that the order observed in this fixed angle system is indeed long-range, with a non-decaying correlation at long length scales.

\begin{figure} [h!]
	\centering
	\includegraphics[width=0.7\linewidth]{Figures/num_fa_correlation_mol}
	\caption{Orientational correlation function over time (as the volume fraction is reduced), where nunchucks have a fixed opening angle \DIFaddbeginFL \DIFaddFL{(between the rigid arms)}\DIFaddendFL . Plotted for a system ofXX}
...
However, no statistically significant periodic components in the correlation function were observed over the length scale of the simulation region. A visual inspection of Figure \ref{fig:nun_fa_views} suggests that the \DIFdelbegin \DIFdel{length of the }\DIFdelend simulation region is approximately \DIFdelbegin \DIFdel{half }\DIFdelend \DIFaddbegin \DIFadd{a factor two shorter than }\DIFaddend the full period of the repeating structure, and so further research on larger simulation regions is required to verify these phases.
...
While the phase behaviour of rigid rods is well studied, much less is known about the dynamical properties \DIFdelbegin \DIFdel{of the individual mesogens inside these quasi-ordered phases}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{of the individual mesogens inside these quasi-rdered phases}}\DIFaddend . This in, in part, due to the traditional popularity of \DIFdelbegin \DIFdel{MC approaches}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{using MC approaches}}\DIFaddend , which cannot predict dynamic properties. Despite this, \DIFdelbegin \DIFdel{dynamic studies have }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{this MC approach} has }\DIFaddend enjoyed a more recent popularity in both computational and experimental studies \cite{GayBalmaz2013, Zhao2013, Rey2013}. Here, we undertake a brief study into the dynamical properties of \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend nunchuck system, and demonstrate the application of dynamical properties to static phase identification. In particular, we focus \DIFdelbegin \DIFdel{upon }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the diffusion coefficient $D$ defined by the evolution of the \DIFdelbegin \DIFdel{mean square }\DIFdelend \DIFaddbegin \DIFadd{mean-squared }\DIFaddend displacement (MSD) \DIFdelbegin \DIFdel{as a function of time $t$ in $n$ }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{as function of time $t$ in $d$} }\DIFaddend dimensions:
...
As the volume fraction is increased, we observe a tendency towards subdiffusive behaviour, with a reduction in the value of $\alpha$\DIFdelbegin \DIFdel{to $0.12 \pm 0.02 $ at $\phi = 0.69$}\DIFdelend . This is given by the gradient when the root \DIFdelbegin \DIFdel{mean squared }\DIFdelend \DIFaddbegin \DIFadd{mean-squared }\DIFaddend (RMS) displacement is plotted over time in logarithmic space, as best demonstrated in Figure \ref{fig:nun_diff_rmsplots}. This depicts coordinate-wise displacement across three different phase regimes, for a \DIFdelbegin \DIFdel{rigid rods system. Here}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{system of rigid rods}. Here, }\DIFaddend the Cartesian axes may be used, as simulations are configured in a crystalline phase with molecules orientated along the $y$\DIFdelbegin \DIFdel{axis}\DIFdelend \DIFaddbegin \DIFadd{-axis}\DIFaddend , and smectic layers forming in the $x-z$ plane. Phase transitions for this system have previously been identified in the regions $0.58<\phi<0.62$ for the smectic\textendash nematic \DIFdelbegin \DIFdel{transition, }\DIFdelend and $0.38<\phi<0.43$ for the nematic\textendash isotropic transition. 

At the highest volume fractions ($\phi \geq 0.62$), the diffusion along the $y$\DIFdelbegin \DIFdel{axis }\DIFdelend \DIFaddbegin \DIFadd{-axis }\DIFaddend is significantly reduced, due to the restricted motion between layers in the smectic phase. Below this, a nematic phase is observed, with increased diffusivity along the \DIFdelbegin \DIFdel{molecule }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{molecule's} }\DIFaddend director (aligned with the $y$-axis) \DIFdelbegin \DIFdel{; }\DIFdelend corresponding to an increased intercept in logarithmic space. \DIFdelbegin \DIFdel{The degree of dynamic anisotropy here, quantified by the ratio of diffusion coefficients $D_{y}/D_{x}$, is $2.12$ for $\phi = 0.58$. }\DIFdelend At the lowest volume fractions ($\phi \leq 0.38$), there is no preferred direction in the system and \DIFdelbegin \DIFdel{motion is isotropic and truly diffusive (as }\DIFdelend \DIFaddbegin \DIFadd{diffusion is completely isotropic. Particle motion in the $x$- and $z$-axis is approximately diffusive throughout (ie }\DIFaddend $\alpha = 1$)\DIFaddbegin \DIFadd{, but the lower intercept at higher volume fractions corresponds to a reduced diffusion coefficient}\DIFaddend . 

\begin{figure} [h!]
	\centering
	\includegraphics[width=\linewidth]{Figures/nun_diff_rmsplots}
	\caption{Root \DIFdelbeginFL \DIFdelFL{mean squared displacement }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mean-squared displacements }\DIFaddendFL against time in a system of 1000 rigid rods \DIFaddbeginFL \DIFaddFL{\textcolor{forestgreen}{with an aspect ratio 10}}\DIFaddendFL , for a range of volume fractions $\phi$. The leftmost pair of plots correspond to the smectic phase, with restricted motion between $x-z$ layers along the $y$-axis. The centre pair give the nematic phase, with preferential displacement along the \DIFdelbeginFL \DIFdelFL{y axis}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$y$-axis}\DIFaddendFL . Finally, the rightmost pair give the isotropic phase, with no preferential direction and isotropic diffusion. Note that there are no significant differences between the $x$\DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL and $z$\DIFdelbeginFL \DIFdelFL{directions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-directions }\DIFaddendFL in any phase, as these directions are equivalent in the phase structure.}
...
Phase formation may also be observed through deviations in these coordinate-specific diffusion coefficients. Applying this to the \DIFdelbegin \DIFdel{nunchucks }\DIFdelend \DIFaddbegin \DIFadd{nunchuck }\DIFaddend system, isotropic diffusion is observed at low volume fractions in Figure \ref{fig:nun_diff_Dcoeff}\DIFdelbegin \DIFdel{. However, }\DIFdelend \DIFaddbegin \DIFadd{, but }\DIFaddend the $y$-coordinate diffusion coefficient is severely reduced \DIFdelbegin \DIFdel{beyond this, with an maximal anisotropy of $D_{y}/D_{x} = 0.19$ at $\phi = 0.58$}\DIFdelend \DIFaddbegin \DIFadd{before this}\DIFaddend . This strongly \DIFdelbegin \DIFdel{indicates the onset of formation of the }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{hints towards the presence of a} }\DIFaddend smectic phase below $\phi = 0.45$, in agreement with the prediction of $ 0.40<\phi^{*}<0.44$ obtained from the smectic order parameter. There is also some indication of the crystalline phase at the highest volume fractions, with diffusion equally limited in all directions, but this evidence is not conclusive.
...
In the fixed angle case, we subsequently introduced the orientational correlation function in order to identify any periodic order in the `herringbone-like' and `\DIFdelbegin \DIFdel{twisted-nematic}\DIFdelend \DIFaddbegin \DIFadd{twisted}\DIFaddend ' quasi-nematic phases observed. While this confirmed the presence of long-range order, we were unable to observe any periodicity here; we \DIFdelbegin \DIFdel{suggest }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{believe} }\DIFaddend that simulations of larger systems will be required due to the expected length scale of such periodic features.

Finally, we demonstrate that the measurement of dynamic properties provide a little-studied\DIFaddbegin \DIFadd{, }\DIFaddend alternative method to identify phase transitions and characteristic symmetries of such systems, in comparison with phases previously identified in both the rigid \DIFdelbegin \DIFdel{rods and nunchucks systemspreviously considered. We consider in particular }\DIFdelend \DIFaddbegin \DIFadd{rod and nunchuck systems. \textcolor{forestgreen}{In particular, I considered} }\DIFaddend the formation of anisotropic phases through the variation in coordinate diffusion coefficients, such as \DIFaddbegin \DIFadd{in }\DIFaddend the smectic phase\DIFdelbegin \DIFdel{where the diffusion coefficient }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{, where the diffusion coefficient of the mesogens }}\DIFaddend within the ordered layers is up to a factor \DIFdelbegin \DIFdel{of }\DIFdelend $5.15$ greater than between them. Further research is necessary to extend this beyond the simple Cartesian approach taken here, to allow the identification of novel phases\DIFaddbegin \DIFadd{, }\DIFaddend where the global director or symmetry class is not known. \DIFdelbegin \DIFdel{Recent results }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{Results, I obtained only very recently,} }\DIFaddend on directional diffusion within the \DIFdelbegin \DIFdel{nunchucks }\DIFdelend \DIFaddbegin \DIFadd{nunchuck }\DIFaddend system also suggest the formation of a novel biaxial phase \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend we are preparing a \DIFdelbegin \DIFdel{separate publication on this}\DIFdelend \DIFaddbegin \DIFadd{publication on these}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \label{sec:NematicOrderAppendix}
%DIFDELCMD < %%%
