...
Since Kirkwood's prediction of an \DIFdelbegin \DIFdel{entropy\textendash }\DIFdelend \DIFaddbegin \DIFadd{entropically }\DIFaddend driven ordering transition in the freezing of hard spheres \cite{Kirkwood1954}, \DIFdelbegin \DIFdel{entropy\textendash }\DIFdelend \DIFaddbegin \DIFadd{entropically }\DIFaddend driven phase transitions have been the subject of much attention \cite{Kerr1993,Frenkel1999}. Their relevance to colloidal systems is often underestimated, but gives rise to a range of diverse phase behaviour \cite{Adams1998, Anderson2002, Forsyth1978}. In particular, there is a wealth of (primarily experimental) studies of `bent-core' liquid crystals - rigid rods with a central bend \cite{Takezoe2006, Etxebarria2008, Yang2018}.
...
. Here I focus lyotropic, entropy\textendash driven phase transitions, where the mesogens (here DNA-nanocolloids) interact purely repulsively. Our studies represent a benchmark for designer-made, DNA\textendash based systems.
...
where $P_{2}$ denotes the second Legendre polynomial \cite{DeGennes1993}. While $\langle \cos^{2}(\theta) \rangle$ would function alone as the order parameter, \DIFdelbegin \DIFdel{this confines $S_{n}$ to values between unity for a perfectly aligned system, and zero for a completely random system. Further information is given in }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{this confines $S_n$ to values between unity for a perfectly aligned system, and zero for a completely random system. Further information is given in} }\DIFaddend Appendix \ref{sec:OrderParamTheory}.
...
%DIF < can cite frenkel on entropy drive transitions if req
...
\DIFdelbegin \DIFdel{We therefore consider systems where the nunchuck opening angle is }\DIFdelend \DIFaddbegin \DIFadd{\textcolor{forestgreen}{Therefore, we considered systems for which the nunchuck opening angle was} }\DIFaddend confined to a particular value\DIFdelbegin \DIFdel{, as we suggest similarly quasi-nematic structures may be observed on shorter length scales,  particularly this opening angle is confined below the mean angle of Figure \ref{fig:nun_fr_angledist}. }\DIFdelend \DIFaddbegin \DIFadd{. \textcolor{forestgreen}{We suggest that similar to the rigid-rods, quasi-nematic structures may be observed on shorter length scales, particularly, when this opening angle is confined below the mean angle (see Figure \ref{fig:nun_fr_angledist})}.
}\DIFaddend We tested phase formation over a wide range of rigid rod lengths and opening angle values; broadly we found that larger opening angles favoured nematic-like phase formation\DIFdelbegin \DIFdel{(with a maximal order parameter of $S_{n} = 0.62$), }\DIFdelend \DIFaddbegin \DIFadd{, \textcolor{forestgreen}{for which} our analysis methods were best suited to identify, }\DIFaddend but smaller angles did not form obvious alternate ordered phases. Two such quasi-nematic phases observed are depicted in Figure \ref{fig:nun_fa_views}.
...
\begin{figure} [h!]
	...
	\caption{Root \DIFdelbeginFL \DIFdelFL{mean squared displacement }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mean-squared displacements }\DIFaddendFL against time in a system of 1000 rigid rods, for a range of volume fractions $\phi$. The leftmost pair of plots correspond to the smectic phase, with restricted motion between $x-z$ layers along the $y$-axis. The centre pair give the nematic phase, with preferential displacement along the y axis. Finally, the rightmost pair give the isotropic phase, with no preferential direction and isotropic diffusion. Note that there are no significant differences between the $x$ and $z$ directions in any phase, as these directions are equivalent in the phase structure.}
